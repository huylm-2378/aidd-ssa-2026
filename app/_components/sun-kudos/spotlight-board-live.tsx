...
export default function SpotlightBoardLive({
  ...
  // Most-recent live recipient — drives the top-left "LIVE" badge and pulses
  // that Sunner's node. `tick` re-triggers the entrance animation each event.
  const [live, setLive] = useState<{ id: string | null; name: string; tick: number } | null>(null);
  ...
  const onInsert = useCallback(
    (row: KudosInsertRow) => {
      ...
      setLive((prev) => ({ id: row.receiver_id, name, tick: (prev?.tick ?? 0) + 1 }));
    },
    [nameById],
  );
  ...
  return (
    <div className="relative isolate overflow-hidden rounded-3xl border border-[#998c5f]/40 bg-[#080c10] px-6 py-8 sm:px-10 sm:py-10">
      <SpotlightBoardBg />
      ...
      <div className="mt-8">
        <SpotlightCanvas nodes={nodes} query={query} live={live} />
      </div>
