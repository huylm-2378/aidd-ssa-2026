...
| Visitor clicks the notification bell icon | Icon remains a static visual affordance; no dropdown/menu content is wired up (unchanged by F005) | "None — bell is inert; not a functional notification feature" |
| Visitor clicks the avatar icon | Real Account Menu opens (F005, Supabase Google OAuth): signed-in visitors see name/email + Sign out; signed-out visitors see a Sign in link to `/login` | Dropdown opens with real auth state — no longer a static stub |
