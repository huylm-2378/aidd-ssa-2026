...
| FR-005 | Người nhận * — recipient search input (placeholder "Tìm kiếm", dropdown arrow); typing filters the DB `sunners` directory (autocomplete over rows fetched server-side via `getSunnerOptions()`, client fallback `useSunnerOptions`; includes real Google members once F007's `0005` link migration is applied); selecting fills the field; required | `RecipientSelect` | yes |
